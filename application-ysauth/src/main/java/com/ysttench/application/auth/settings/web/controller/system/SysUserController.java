...
public class SysUserController extends BaseController {
    ...
    public String listUIE(Model model) throws Exception {
        return ForwardConstants.SYSTEM + ForwardConstants.USER + ForwardConstants.LIST2;
    }
    @ResponseBody
    ...
    public void download(HttpServletRequest request, HttpServletResponse response, String column,String sort) throws IOException {
        ...
        for (SysUserFormMap sysUserFormMap : lis) {
            if(sysUserFormMap.getStr("deleteStatus").equals("0")){
                sysUserFormMap.put("deleteStatus", "可用");
            }else{
                sysUserFormMap.put("deleteStatus", "禁用");
            }
        }
    ...
    public String addEntity(String txtGroupsSelect) {
        try {
            if (txtGroupsSelect == null || txtGroupsSelect.equals("")) {
                ...
                if ("0".equals(sysUserFormMap.getStr("userType"))) {
                ...
                if ("驭势科技（浙江）有限公司".equals(sysUserFormMap.get("organ").toString())) {
                	sysUserFormMap.put("organId", "100002");
				}else if("驭嘉贸易（浙江）有限公司".equals(sysUserFormMap.get("organ").toString())) {
					sysUserFormMap.put("organId", "100003");
				}else if("驭势（上海）汽车科技有限公司".equals(sysUserFormMap.get("organ").toString())) {
					sysUserFormMap.put("organId", "100001");
				}else if("驭势科技（北京）有限公司".equals(sysUserFormMap.get("organ").toString())) {
					sysUserFormMap.put("organId", "1");
				}
                ...
                if (!StringUtil.isEmpty(txtGroupsSelect)) {
                    ...
                    for (String roleId : txt) {
                        userGroupsFormMap.put("userId", sysUserFormMap.get("userId"));
                        userGroupsFormMap.put("roleId", roleId);
                        sysUserMapper.addEntityru(userGroupsFormMap);
                    }
                }
            }
        } catch (Exception e) {
    ...
    public String editEntity(String txtGroupsSelect) throws Exception {
        if (txtGroupsSelect == null || txtGroupsSelect.equals("")) {
           ...
           // userFormMap.put("createTime", DatetimeUtil.getDate());
            userFormMap.put("txtGroupsSelect", txtGroupsSelect);
            if ("驭势科技（浙江）有限公司".equals(userFormMap.get("organ").toString())) {
            	userFormMap.put("organId", "100002");
			}else if("驭嘉贸易（浙江）有限公司".equals(userFormMap.get("organ").toString())) {
				userFormMap.put("organId", "100003");
			}else if("驭势（上海）汽车科技有限公司".equals(userFormMap.get("organ").toString())) {
				userFormMap.put("organId", "100001");
			}else if("驭势科技（北京）有限公司".equals(userFormMap.get("organ").toString())) {
				userFormMap.put("organId", "1");
			}
    ...
    public String editPassword() throws Exception {
        ...
        List<SysUserFormMap> dataOne = sysUserMapper.findUserPage(oneUser);
        // 这里对修改的密码进行加密
