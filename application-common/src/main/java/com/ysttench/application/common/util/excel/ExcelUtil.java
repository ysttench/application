...
//        f3.setColor(IndexedColors.RED.getIndex());
        ...
        //设置每行每列的值(excel的上限为65535)
        for (int i = 0; list != null && i < list.size() && i <= 65535; i++) {
     ...
     * @throws Exception
     */
    public static Workbook createExcelFile(InputStream inputStream, String fileType){
        Workbook wb = null;
        if (ExcelConstants.XLS.equals(fileType)) {
            try {
                wb = new HSSFWorkbook(inputStream);
            } catch (IOException e) {
                logger.error("failed parse io for poi");
            }
        ...
        if (wb == null || wb.getNumberOfSheets() == 0) {
            logger.error("not found sheet or invalid upload file");
        }
        return wb;
    }
