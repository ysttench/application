package com.ysttench.application.htdytasties.settings.common;
...
public class TreeUtil {
	...
	// 得到子节点列表
	List<MenuTreeObject> childList = getChildList(list, t);
	...
	// 得到子节点列表
	List<MenuTreeObject> childList = getChildList(list, node);
	// 判断是否有子节点
	...
	/*
	 * long start = System.currentTimeMillis(); List<TreeObject> TreeObjectList =
	 * new ArrayList<TreeObject>();
	 * 
	 * TreeObjectUtil mt = new TreeObjectUtil(); List<TreeObject>
	 * ns=mt.getChildTreeObjects(TreeObjectList,0); for (TreeObject m : ns) {
	 * System.out.println(m.getName()); System.out.println(m.getChildren()); } long
	 * end = System.currentTimeMillis(); System.out.println("用时:" + (end - start) +
	 * "ms");
	 */
    }
